...
class BughouseStudyTool {
    ...
    movePiece(boardId, from, to) {
        ...
        this.recordMove(boardId, from, to, piece, capturedPiece, false, this.isKingInCheck(boardId, board.turn), board.checkmate, null);
        this.updatePools();
        this.renderBoards();
        return true;
    }
    ...
    updateGameState(boardId) {
        ...
        if (!hasMoves) {
            if (board.check) {
                ...
                this.stopBotMoves();
                this.gameActive = false;
                this.updateStatus(`♟️ STALEMATE! Draw on Board ${boardId.toUpperCase()}! ♟️`);
    ...
    calculateLegalMoves(boardId, squareCoord, piece) {
        ...
        for (const [ts, mt] of rawMoves) {
            ...
            if (!this.isKingInCheckAfterMove(simBoard, piece.color)) {
                ...
                if (!piece.hasMoved && !board.check) {
                    ...
                    if (file === 4 && rank === sr2) {
                        if (cr.kingside && !board.position.has(`5,${sr2}`) && !board.position.has(`6,${sr2}`) &&
                            !this.isSquareAttacked(boardId, `4,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
                            !this.isSquareAttacked(boardId, `5,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
                            !this.isSquareAttacked(boardId, `6,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
                        ...
                        if (cr.queenside && !board.position.has(`1,${sr2}`) && !board.position.has(`2,${sr2}`) && !board.position.has(`3,${sr2}`) &&
                            !this.isSquareAttacked(boardId, `4,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
                            !this.isSquareAttacked(boardId, `3,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
                            !this.isSquareAttacked(boardId, `2,${sr2}`, piece.color === 'white' ? 'black' : 'white') &&
    ...
    simulateMove(boardId, from, to, piece, moveType) {
        ...
        if (moveType === 'en-passant') { const [ff, fr] = from.split(',').map(Number); const [tf, tr] = to.split(',').map(Number); sim.delete(`${tf},${fr}`); }
            ...
            if (moveType === 'castle-kingside') { sim.delete(`7,${rk}`); sim.set(`5,${rk}`, { type: 'rook', color: piece.color, hasMoved: true }); }
    ...
    isKingInCheckAfterMove(simBoard, color) {
        let kingSq = null;
            ...
            for (let f = 0; f < 8; f++) {
                for (let r = 0; r < 8; r++) {
                    ...
                    for (const piece of pool) {
                        ...
                        if (!this.isKingInCheckAfterMove(simBoard, color)) {
                            return true;
                        }
                        break;
                    }
    ...
    performRedo(action) {
        ...
        if (action.type === 'move') {
            ...
            if (action.moveType === 'en-passant') {
                ...
                board.position.delete(action.from);
                board.position.set(action.to, { ...piece, hasMoved: true });
                if (action.moveType === 'castle-kingside') { board.position.delete(`7,${rk}`); board.position.set(`6,${rk}`, { type: 'rook', color: piece.color, hasMoved: true }); }
                else { board.position.delete(`0,${rk}`); board.position.set(`3,${rk}`, { type: 'rook', color: piece.color, hasMoved: true }); }
    ...
    performUndo(action) {
        ...
        if (action.type === 'move') {
            ...
            if (action.moveType?.includes('castle')) {
                ...
                if (action.moveType === 'castle-kingside') { board.position.delete(`6,${rk}`); board.position.set(`7,${rk}`, { type: 'rook', color: action.piece.color, hasMoved: false }); }
                else { board.position.delete(`2,${rk}`); board.position.set(`0,${rk}`, { type: 'rook', color: action.piece.color, hasMoved: false }); }
            }
    ...
    exportCSV() {
        ...
        for (let i = 0; i < this.moveHistory.length; i++) {
            ...
            const endSq = this.coordToNotation(m.to, m.board);
            let special = '';
