// ============================================================
// BUGHOUSE CLASSIC - Adam Scarchilli Study Tool
...
class BughouseStudyTool {
            ...
            for (let visualRank = 0; visualRank < 8; visualRank++) {
                for (let file = 0; file < 8; file++) {
                    ...
                    if (kingSquare === square && board.check) {
                        squareDiv.classList.add('check', 'sq-check-hint');
                    }
                    if (kingSquare === square && board.checkmate) {
                        squareDiv.classList.add('sq-mate-hint');
                    }
    ...
    movePiece(boardId, from, to) {
        ...
        this.updatePools();
        this.renderBoards();
        // Show move result in status box
        const fromN = this.coordToNotation(from, boardId);
        const toN = this.coordToNotation(to, boardId);
        const pieceName = piece.type.charAt(0).toUpperCase() + piece.type.slice(1);
        const boardLabel = boardId.toUpperCase();
        if (board.checkmate) {
            // checkmate message already set by updateGameState
        } else if (board.check) {
            this.updateStatus(`⚠️ CHECK! ${piece.color === 'white' ? 'White' : 'Black'} played ${fromN}-${toN} on Board ${boardLabel} — ${board.turn.charAt(0).toUpperCase() + board.turn.slice(1)} is in check!`);
        } else {
            this.updateStatus(`${piece.color === 'white' ? 'White' : 'Black'} played ${pieceName} ${fromN}-${toN} on Board ${boardLabel}`);
        }
        return true;
    }
    ...
    completePromotion(boardId, from, to, piece, promotedType) {
        ...
        this.updatePools();
        this.renderBoards();
        const promFrom = this.coordToNotation(from, boardId);
        const promTo = this.coordToNotation(to, boardId);
        const promBoard = boardId.toUpperCase();
        if (board.checkmate) {
            // checkmate message already set
        } else if (board.check) {
            this.updateStatus(`⚠️ CHECK! ${piece.color === 'white' ? 'White' : 'Black'} promoted pawn ${promFrom}-${promTo}=${promotedType} on Board ${promBoard} — ${board.turn.charAt(0).toUpperCase() + board.turn.slice(1)} in check!`);
        } else {
            this.updateStatus(`${piece.color === 'white' ? 'White' : 'Black'} promoted pawn ${promFrom}-${promTo}=${promotedType} on Board ${promBoard}`);
        }
    }
            ...
            case 'rook': case 'bishop': case 'queen': {
                ...
                for (const [df, dr] of dirs) {
                    ...
                    while (iv(f, r)) {
                        ...
                        const t = board.position.get(ts);
                        if (t && t.type === 'king') {
                            // Include king square so isKingInCheck can detect attacks through raw moves
                            if (t.color !== piece.color) moves.set(ts, 'capture');
                            break;
                        }
                        if (!t) {
                            moves.set(ts, 'normal');
    ...
    dropPiece(boardId, squareCoord) {
        ...
        this.updatePools();
        this.renderBoards();
        const dropN = this.coordToNotation(squareCoord, boardId);
        const dropBoard = boardId.toUpperCase();
        if (board.checkmate) {
            // checkmate message already set by updateGameState
        } else if (board.check) {
            this.updateStatus(`⚠️ CHECK! ${dp.color === 'white' ? 'White' : 'Black'} dropped ${dp.type} at ${dropN} on Board ${dropBoard} — ${board.turn.charAt(0).toUpperCase() + board.turn.slice(1)} is in check!`);
        } else {
            this.updateStatus(`${dp.color === 'white' ? 'White' : 'Black'} dropped ${dp.type} at ${dropN} on Board ${dropBoard}`);
        }
    }
